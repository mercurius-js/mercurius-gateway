...
function validateGateway(opts) {
  ...
  if (opts.schema || opts.resolvers || opts.loaders) {
    throw new MER_ERR_INVALID_OPTS(
      'Adding "schema", "resolvers" or "loaders" to plugin options when plugin is running in gateway mode is not allowed'
    )
  }
...
async function newInitGateway(gatewayOpts, app) {
  ...
  app.addHook('onReady', async function () {
    onAppReady(app.graphql)
  })

  function onAppReady(fastifyGraphQl) {
