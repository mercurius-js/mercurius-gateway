...
const { getNamedType, isObjectType, isScalarType, Kind } = require('graphql')
...
function defineResolvers(
  ...
  for (const type of Object.values(types)) {
    // Exclude the scalar types and the default types (eg     '__Schema','__Type'...)
      ...
      for (const field of Object.values(type.getFields())) {
        ...
        if (!isScalarType(fieldType)) {
          ...
          if (
            ...
            (serviceForFieldType !== null &&
              serviceForType !== null &&
              serviceForFieldType === serviceForType)
          ) {
            /**
             ...
             */
            let service =
              serviceMap[typeFieldsToService[`${type}-${fieldName}`]]
            if (
              !service &&
              (type.name === 'Query' ||
                type.name === 'Mutation' ||
            ...
            if (!service) {
              ...
              if (!service) {
                ...
                const leafKind = isNonNull
                  ? field.astNode.type.type.kind
              ...
              field.subscribe = makeResolverSubscription({
                service,
                entityResolversFactory,
                lruGatewayResolvers
              })
            } else {
              field.resolve = makeResolverQuery({
                service,
          ...
          /*
             Is scalar but has a mapping to a service (eg. a query that returns a string)
             extend type Query {
               foo: String!
             }
           */
...
async function buildGateway(serviceMap, gatewayOpts, app, lruGatewayResolvers) {
  ...
  const serviceSDLs = Object.entries(serviceMap).reduce(
    (acc, [name, value]) => {
      ...
      error !== null
        ? app.log.warn(
            `Initializing service "${name}" failed with message: "${error.message}"`
          )
  ...
  const schema = buildFederatedSchema(serviceSDLs.join(''), { isGateway: true })
  ...
  /**
   * Create a mapper for the resolution of the resolvers
   *
   * typeToServiceMap = { User: 'user', Post: 'post' }
   * typeFieldsToService = { 'Query-me': 'user', 'User-topPosts': 'post' }
   */
  for (const [service, serviceDefinition] of Object.entries(serviceMap)) {
    ...
    for (const [type, fields] of Object.entries(
      serviceDefinition.extensionTypeMap
    )) {
      // The extended types can be defined just once, if more than one service declare the same attribute this error is thrown
      // Object.checkFieldUniqueness
      //         (node_modules/graphql/validation/rules/UniqueFieldDefinitionNamesRule.js:61:11)
  ...
  return {
    ...
    async refresh(isRetry) {
      ...
      if (failedMandatoryServices.length > 0) {
        const serviceNames = failedMandatoryServices.map(
      ...
      const schema = buildFederatedSchema(_serviceSDLs, { isGateway: true })
