...
const { collect, resolvePath } = require('../collectors')
...
function makeResolver ({
  ...
  return async function (parent, args, context, info) {
    ...
    if (isQuery) {
      ...
      if (service.collectors) {
        collect(service.collectors, context, {
          key: resolvePath(info.path), response
        })
      }
      ...
      if (typeToServiceMap) {
        ...
        if (targetService && targetService !== service.name) {
          ...
          const toFill = Array.isArray(transformed)
            ? transformed
            : [transformed]
