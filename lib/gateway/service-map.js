...
function createTypeMap (schemaDefinition) {
  ...
  for (const definition of parsedSchema.definitions) {
    ...
    if (isTypeDefinitionNode(definition) && !isTypeExtensionByDirective) {
      typeMap[definition.name.value] = createFieldSet(
        typeMap[definition.name.value],
        definition
      )
      ...
      extensionTypeMap[definition.name.value] = createFieldSet(
        extensionTypeMap[definition.name.value],
        definition,
...
async function buildServiceMap (serviceMap, services, errorHandler, log) {
  for (const service of services) {
    const {
      mandatory = false,
      initHeaders,
      useSecureParse = false,
      ...opts
    } = service
    ...
    const serviceConfig = {
      ...
      setResponseHeaders: reply =>
        opts.setResponseHeaders ? opts.setResponseHeaders(reply) : null,
      close,
      async refresh () {
        ...
        const { schema, schemaDefinition, typeMap, types, extensionTypeMap } =
      ...
      async init () {
        const schemaDefinition =
          opts.schema ||
          (await getRemoteSchemaDefinition(serviceConfig, initHeaders))

        const { typeMap, types, extensionTypeMap } =
          createTypeMap(schemaDefinition)
  ...
  const mapper = async service => {
    ...
    try {
      ...
      if (!service.mandatory || err instanceof GraphQLError) {
        log.warn(
          `Initializing service "${service.name}" failed with message: "${err.message}"`
        )
    ...
    serviceMap[service.name].collectors = service.collectors
  }
