...
test('gateway resolves union type with entity members defined on separate services', async t => {
  ...
  const [reviewService, reviewServicePort] = await createTestService(
    t,
    `
  ...
  const [searchService, searchServicePort] = await createTestService(
    t,
    `
    extend type Query {
      search: [SearchResult]
...
test('gateway resolves nested union array fields from different services', async t => {
  ...
  const [digitalService, digitalServicePort] = await createTestService(
    t,
    `
  ...
  const [storeService, storeServicePort] = await createTestService(
    t,
    `
    extend type Query {
      orders: [Order]
...
test('gateway propagates null for singular top-level union when entity resolver returns null', async t => {
  ...
  const [searchService, searchServicePort] = await createTestService(
    ...
    `,
    {
      Query: {
        topResult: () => {
          return { __typename: 'Article', id: 'a_missing' }
        }
      },
      SearchResult: {
        resolveType (obj) {
  ...
  const [storeService, storeServicePort] = await createTestService(
    ...
    `,
    {
      Query: {
        order: () => ({
          id: '1',
          item: { __typename: 'Product', id: 'p_missing' }
        })
      },
      Purchasable: {
        resolveType (obj) {
...
test('gateway forwards query variables to cross-service union entity queries', async t => {
  ...
  const query = `
    query SearchQuery($skipTitle: Boolean!) {
      search {
        ... on Article {
          id
  ...
  t.same(JSON.parse(res1.body), {
    data: {
      search: [
        ...
        { id: 'r1', rating: 5, body: 'Excellent!' },
        { id: 'a2', title: 'Fastify Performance Tips' },
        { id: 'r2', rating: 3, body: 'Average' }
      ]
    }
  })
...
test('gateway forwards query variables to nested cross-service union entity queries', async t => {
  ...
  const query = `
    query OrdersQuery($skipPrice: Boolean!) {
      orders {
        id
        items {
          ... on Product {
      ...
      orders: [
        {
          ...
          items: [
            ...
            { id: 'd1', title: 'E-Book', downloadUrl: 'https://example.com/ebook' }
          ]
        }
      ]
    }
